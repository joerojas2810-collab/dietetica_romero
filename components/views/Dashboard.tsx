...
import {
  ...
  Sparkles, TrendingUp, Wallet, Building2,
...
import { money, compactMoney } from '@/lib/helpers';
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { movimientos: movs, gastosFijos: gf, apertura: ap } = 
        ...
        <div className="mt-5 grid gap-5 xl:grid-cols-[1.55fr_1fr]">
          <div className="rounded-3xl border border-[#e5eae1] bg-white p-5 shadow-[0_8px_30px_rgba(65,82,55,0.04)] sm:p-6">
            <div className="mb-4 flex items-start justify-between">
              <div>
                <p className="text-sm font-bold">Resumen de ventas</p>
                <p className="mt-1 text-xs text-[#99a398]">Evolución del mes</p>
              </div>
            </div>
          ...
          {/* ── PANEL ÚLTIMOS MOVIMIENTOS (100% real, sin cálculos) ────── */}
          <div className="rounded-3xl bg-[#40562a] p-6 text-white shadow-xl shadow-[#40562a20]">
            <div className="flex items-start justify-between">
              <div>
                <p className="text-sm font-bold">Últimos Movimientos</p>
                <p className="mt-1 text-xs text-[#c0d7bd]">Transacciones más recientes del mes</p>
              </div>
            </div>
            
            <div className="mt-6 space-y-3">
              {ultimosMovimientos.length === 0 ? (
                <div className="flex h-48 items-center justify-center text-xs text-[#c0d7bd]">
                  Sin movimientos registrados este mes
                </div>
              ) : (
                ultimosMovimientos.map((m) => {
                  const esEntrada = Number(m.entrada) > 0;
                  const monto = esEntrada ? Number(m.entrada) : Number(m.salida);
                  
                  return (
                    <div key={m.id} className="flex items-center justify-between rounded-xl bg-white/10 p-3">
                      <div className="flex items-center gap-2.5">
                        <span className={`flex h-7 w-7 items-center justify-center rounded-lg ${
                          esEntrada ? 'bg-[#eff8ed] text-[#3d6942]' : 'bg-[#fdf0ee] text-[#ba4a3a]'
                        }`}>
                          {esEntrada ? <ArrowDownLeft size={14} /> : <ArrowUpRight size={14} />}
                        </span>
                        <div className="min-w-0">
                          <p className="truncate text-xs font-bold">{m.concepto}</p>
                          <p className="text-[10px] text-[#c0d7bd]">
                            {format(parseISO(m.fecha), "d 'de' MMM", { locale: es })} · {m.metodo}
                          </p>
                        </div>
                      </div>
                      <span className={`text-xs font-bold ${
                        esEntrada ? 'text-[#bdddb9]' : 'text-[#f6c2be]'
                      }`}>
                        {esEntrada ? '+' : '-'}{money(monto)}
                      </span>
                    </div>
                  );
                })
