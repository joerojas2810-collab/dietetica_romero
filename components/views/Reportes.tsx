...
export default function Reportes() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const { saldos, movimientos, arqueos } = await db.getReportesData(inicio, fin);

      setAperturas(saldos);

      // Calcular saldos acumulados por día y método
      ...
      // Comparar arqueos reales vs movimientos del día
      const lista: Desvio[] = arqueos.map(a => {
        const s = porDia[a.fecha] || { ef: 0, db: 0, mp: 0 };

        const difEfectivo = a.total_contado != null
          ? Number(a.total_contado) - s.ef
          : null;

        const difDebito = a.disponible_debito != null
          ? Number(a.disponible_debito) - s.db
          : null;

        const difMP = a.disponible_mp != null
          ? Number(a.disponible_mp) - s.mp
          : null;
      ...
      // Filtrar solo días con algún desvío
      setDesvios(
        lista.filter(d =>
            ...
            <div className="space-y-3">
              {desvios.map(d => (
                <div key={d.fecha} className="rounded-xl border border-[#edf0eb] p-4">
                  ...
                  {d.observaciones && (
                    <p className="mt-3 rounded-lg bg-[#f5f5ec] px-3 py-2 text-[11px] text-[#849083]">
                      📝 {d.observaciones}
                    </p>
                  )}
                </div>
