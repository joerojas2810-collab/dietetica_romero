'use client';import Login from '@/components/Login';
...
import {
  ...
  Filter, Menu, MoreHorizontal, Pencil, PiggyBank, Receipt,
  Search, Settings2, ShieldCheck, SlidersHorizontal,
...
import { supabase, Movimiento, ArqueoDiario, MetodoPago, CategoriaGasto } from '@/lib/supabase';
...
export default function Home() {
  ...
  return (
    ...
    <main className="min-h-screen bg-[#f6f7f2] text-[#243126]">
      {/* Sidebar */}
      <aside className={`fixed inset-y-0 left-0 z-40 flex w-[248px] flex-col border-r border-[#e5e9df] bg-[#fbfcf8] px-5 py-6 transition-transform duration-300 lg:translate-x-0 overflow-y-auto ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>        <div className="mb-10 flex items-center justify-between px-2">
...
{view === 'dashboard' ? 'Resumen del negocio' : view === 'day' ? 'Carga del día' : view === 'movements' ? 'Movimientos' : view === 'cartuchera' ? 'Control de cartuchera' : view === 'controlmp' ? 'Control MercadoPago' : view === 'controldebito' ? 'Control Débito' : 'Pagos individuales'}              </h1>
            </div>
          </div>
  ...
  <div className="rounded-3xl border border-[#e5eae1] bg-white p-5 shadow-[0_8px_30px_rgba(65,82,55,0.04)] sm:p-6">
    <div className="mb-4 flex items-start justify-between">
      <div>
        ...
        <p className="mt-1 text-xs text-[#99a398]">Ventas vs Gastos vs Resultado</p>
      </div>
    </div>
...
function DayForm({ onSave }: { onSave: (msg: string) => void }) {
  ...
  const [dispMP, setDispMP] = useState('');
  const [dispDebito, setDispDebito] = useState('');
...
const totalContado = val100 + val200 + val500 + val1000 + val2000 + cajaFuerteTotal;
  ...
  const handleGuardar = async () => {
    setSaving(true);
    try {
      // 1. Borrar movimientos del día (idempotente)
      await supabase.from('movimientos').delete().eq('fecha', fecha);
      ...
      if (cobros.length + gastosRows.length > 0) {
        await supabase.from('movimientos').insert([...cobros, ...gastosRows]);
      }

      // 4. Upsert arqueo
await supabase.from('arqueo_diario').upsert({
  fecha,
  bill_100: val100,
  bill_200: val200,
  bill_500: val500,
    ...
    <p className="mt-1 text-2xl font-bold text-[#4f8755]">
      {difEfectivo === 0 ? '✓ Sin diferencia' : difEfectivo > 0 ? `+${money(difEfectivo)}` : `-${money(Math.abs(difEfectivo))}`}
          ...
          <SectionCard number="03" title="Gastos del día" description="Sumá cada gasto realizado durante la jornada.">
            <div className="space-y-3">
              {gastos.map((row, i) => (
                <div key={i} className="grid gap-2 rounded-xl border border-[#e8ede5] bg-[#fbfcfa] p-3 sm:grid-cols-[1.3fr_0.7fr_0.8fr_0.9fr_auto] sm:items-center sm:border-0 sm:bg-transparent sm:p-0">
                  <input
                    value={row.concept}
                    onChange={e => setGastos(gastos.map((g, j) => j === i ? { ...g, concept: e.target.value } : g))}
                  ...
                  <input
                    value={row.amount}
                    onChange={e => setGastos(gastos.map((g, j) => j === i ? { ...g, amount: e.target.value } : g))}
                    placeholder="$ 0"
                    inputMode="numeric"
                    className="h-11 rounded-lg border border-[#e2e8df] bg-white px-3 text-sm outline-none focus:border-[#9ab498]"
                  />
                  <select
                    value={row.method}
                    onChange={e => setGastos(gastos.map((g, j) => j === i ? { ...g, method: e.target.value as MetodoPago } : g))}
                  ...
                  <select
                    value={row.category}
                    onChange={e => setGastos(gastos.map((g, j) => j === i ? { ...g, category: e.target.value as CategoriaGasto } : g))}
                    className="h-11 rounded-lg border border-[#e2e8df] bg-white px-3 text-xs outline-none"
                  >
            ...
            <button
              onClick={() => setGastos([...gastos, { concept: '', amount: '', method: 'Efectivo', category: 'OPERATIVO' }])}
...
const diferencia = totalCartucheraReal - totalSistema;
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const total = (data || []).reduce((s, r) => s + Number(r.a_caja_fuerte), 0);
  ...
  const handleGuardar = async () => {
    ...
    await supabase.from('movimientos').insert({
      ...
      concepto: diferencia > 0
        ? `Sobrante cartuchera: ${observacion}`
        : `Faltante cartuchera: ${observacion}`,
...
function ControlMP() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const ingresos = (movs || []).reduce((s, m) => s + Number(m.entrada), 0);
      const egresos = (movs || []).reduce((s, m) => s + Number(m.salida), 0);
      setIngresosMP(ingresos);
      setEgresosMP(egresos);
      setLoading(false);
      setSaved(false);
    };
    fetchData();
  ...
  const handleGuardar = async () => {
    ...
    if (diferencia !== 0) {
      await supabase.from('movimientos').insert({
        fecha,
        concepto: diferencia > 0
          ? 'Diferencia positiva MP'
          : 'Diferencia negativa MP',
        entrada: diferencia > 0 ? Math.abs(diferencia) : 0,
        salida: diferencia < 0 ? Math.abs(diferencia) : 0,
        ...
        <div className="rounded-3xl border border-[#e5eae1] bg-white p-6 shadow-[0_8px_30px_rgba(65,82,55,0.04)]">
          ...
          {n(cierreHoy) > 0 && (
            <>
              ...
              {/* Diferencia negativa: sugerir pagos faltantes */}
              {diferencia < 0 && (
                <div className="mt-3 rounded-xl border border-[#f0b9b3] bg-[#fdf0ee] p-4">
                  ...
                  <p className="mt-1 text-[11px] text-[#ba7665]">
                    Verificá si hay pagos o gastos en MP que no se cargaron al sistema.
                    Podés agregarlos desde la sección "Pagos individuales".
                  </p>
                </div>
              )}

              {/* Diferencia positiva: informar */}
              {diferencia > 0 && (
                <div className="mt-3 rounded-xl border border-[#e8c96e] bg-[#fef9e7] p-4">
                  ...
                  <p className="mt-1 text-[11px] text-[#926c00]">
                    Hay más plata de la esperada. Se registrará como diferencia positiva.
                  </p>
                </div>
              )}
...
function ControlDebito() {
  ...
  const handleGuardar = async () => {
    ...
    await supabase.from('arqueo_diario').upsert({
      fecha,
      disponible_debito: n(acreditacionReal),
    }, { onConflict: 'fecha' });
    ...
    if (diferencia !== 0) {
      ...
      await supabase.from('movimientos').insert({
        ...
        entrada: diferencia > 0 ? Math.abs(diferencia) : 0,
        salida: diferencia < 0 ? Math.abs(diferencia) : 0,
        metodo: 'Debito' as MetodoPago,
        categoria: (esComision ? 'OPERATIVO' : 'EXTRAORDINARIO') as CategoriaGasto,
      });
    }
        ...
        <div className="rounded-3xl border border-[#e5eae1] bg-white p-6 shadow-[0_8px_30px_rgba(65,82,55,0.04)]">
          ...
          {n(acreditacionReal) > 0 && (diferencia === 0 || tipo) && (
            <button
              ...
              className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-[#40562a] py-3 text-sm font-bold text-white transition hover:bg-[#30431f] disabled:opacity-60"
            >
              {saving ? 'Guardando...' : 'Confirmar control Débito'}
              <ShieldCheck size={16} />
            </button>
          )}
  ...
  return (
    <div className="animate-in fade-in duration-500">
      <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
        <div>
          <div className="mb-2 flex items-center gap-2 text-xs font-semibold text-[#5d713c]">
            <CirclePlus size={14} /> Registro individual
          </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1fr]">
        ...
        <div className="rounded-3xl border border-[#e5eae1] bg-white p-6 shadow-[0_8px_30px_rgba(65,82,55,0.04)]">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`rounded-xl border-2 p-3 text-center text-sm font-bold transition ${
                  tipo === 'salida' ? 'border-[#ba4a3a] bg-[#fdf0ee] text-[#ba4a3a]' : 'border-[#e5eae1] text-[#849083]'
                }`}
              >
                ↑ Gasto / Pago
              </button>
              <button
                ...
                className={`rounded-xl border-2 p-3 text-center text-sm font-bold transition ${
                  tipo === 'entrada' ? 'border-[#40562a] bg-[#edf0e2] text-[#40562a]' : 'border-[#e5eae1] text-[#849083]'
                }`}
              >
                ↓ Cobro / Ingreso
              </button>
              ...
              {(['Efectivo', 'Debito', 'MercadoPago'] as MetodoPago[]).map(m => (
                <button
                  ...
                  className={`rounded-xl border-2 py-2 text-xs font-bold transition ${
                    metodo === m ? 'border-[#40562a] bg-[#edf0e2] text-[#40562a]' : 'border-[#e5eae1] text-[#849083]'
                  }`}
                >
                  {m === 'MercadoPago' ? 'MP' : m}
                </button>
          ...
          <button
            ...
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-[#40562a] py-3 text-sm font-bold text-white transition hover:bg-[#30431f] disabled:opacity-60"
          >
              ...
              <tbody>
                {filtered.map((m) => (
                  <tr key={m.id} className="border-b border-[#f0f2ee] transition hover:bg-[#fbfcfa]">
                    ...
                    <td className="px-6 py-4">
                      <button onClick={() => handleDelete(m.id!)} className="text-[#a6b0a5] hover:text-[#ba4a3a]">
                        <Trash2 size={15} />
                      </button>
                    </td>
                  </tr>
